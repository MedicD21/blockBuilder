...
async function createSchema() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS housing_configs (
      id TEXT PRIMARY KEY,
      user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      name TEXT NOT NULL,
      config JSONB NOT NULL,
      happiness_score NUMERIC NOT NULL DEFAULT 0,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );
  `;
  ...
  await sql`
    CREATE INDEX IF NOT EXISTS housing_configs_user_idx
    ON housing_configs (user_id, updated_at DESC);
  `;
}
