...
export function buildPreconfiguredHousingPlans(pokemonDataset) {
  ...
  return Array.from(grouped.entries())
    .map(([idealHabitat, residents]) => {
      ...
      const sampleResidentProfiles = sortedResidents.slice(0, 4).map((resident) => ({
        name: resident.name,
        spriteUrl: resident.meta?.spriteUrl || "",
        favorites: Array.isArray(resident.favorites) ? resident.favorites.slice(0, 4) : [],
      }));
...
function calculateResidentScore({
  ...
  const favoriteRatio = favorites.length > 0 ? favoriteHits / favorites.length : 0.55;
  const attractionRatio =
    attractions.length > 0 ? attractionHits / attractions.length : 0.5;

  const favoriteScore = favorites.length > 0 ? 55 + favoriteRatio * 45 : 72;

  const attractionScore = attractions.length > 0 ? 50 + attractionRatio * 50 : 70;
  ...
  const score =
    favoriteScore * 0.5 +
    preferredHabitatScore * 0.16 +
    habitatSpawnScore * 0.14 +
...
export function calculateHousingComfort({
  selectedResidents,
  selectedItems,
  preferredHabitat,
  selectedHabitat,
  ...
  const residentFavoriteSet = new Set();
  ...
  safeResidents.forEach((resident) => {
    for (const favorite of resident.favorites || []) {
      residentFavoriteSet.add(normalizeKey(favorite));
    }
  });
  ...
  const occupancyScore = clamp(60 + occupancyRatio * 40, 0, 100);
  const matchedFavoriteCount = Array.from(residentFavoriteSet).filter((favorite) =>
    selectedFavoriteSet.has(favorite),
  ).length;
  const favoriteCoverageScore =
    residentFavoriteSet.size > 0
      ? clamp((matchedFavoriteCount / residentFavoriteSet.size) * 100, 0, 100)
      : 60;
  const varietyScore =
    selectedItemSectionSet.size > 0
      ? clamp(35 + (selectedItemSectionSet.size / 4) * 65, 0, 100)
      : 35;
  const itemsPerResident =
    safeResidents.length > 0
      ? totalPlacedItems / safeResidents.length
      : totalPlacedItems;
  const itemDensityScore = clamp((itemsPerResident / 3) * 100, 0, 100);

  const totalBlocks = Number(linkedBuilderProject?.totalBlocks || 0);
  const builderBonus = totalBlocks > 0 ? Math.min(6, Math.log10(totalBlocks + 1) * 2) : 0;

  const comfort =
    residentAverage * 0.74 +
    occupancyScore * 0.07 +
    favoriteCoverageScore * 0.1 +
    varietyScore * 0.04 +
    itemDensityScore * 0.03 +
    builderBonus;

  return {
    score: Number(clamp(comfort, 0, 100).toFixed(1)),
    ...
    itemDensityScore: Number(itemDensityScore.toFixed(1)),
...
export const calculateHousingHappiness = calculateHousingComfort;
