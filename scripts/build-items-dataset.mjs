...
const OUTPUT_PATH = path.join(ROOT, 'data', 'items-dataset.json');
...
async function attachFavoriteTags(sections, favoriteOrder) {
  ...
  const favoriteFiles = (await fs.readdir(FAVORITES_DIR_PATH)).filter((file) =>
    file.toLowerCase().endsWith('.md'),
  );

  const favoriteNames = new Set(favoriteOrder);

  for (const fileName of favoriteFiles) {
...
function sortFavoriteTypes(favoriteTypes, preferredOrder) {
  ...
  return favoriteTypes.sort((a, b) => {
    const aIndex = orderIndex.has(a)
      ? orderIndex.get(a)
      : Number.MAX_SAFE_INTEGER;
    const bIndex = orderIndex.has(b)
      ? orderIndex.get(b)
      : Number.MAX_SAFE_INTEGER;
...
async function buildItemsDataset() {
  const [itemsHtml, favoritesIndex] = await Promise.all([
    fs.readFile(ITEMS_HTML_PATH, 'utf8'),
    fs.readFile(FAVORITES_INDEX_PATH, 'utf8'),
  ]);

  const sections = parseItemsHtml(itemsHtml);
  const favoriteOrder = parseFavoriteOrder(favoritesIndex);
  const { favoriteTypes, unmatched } = await attachFavoriteTags(
    sections,
    favoriteOrder,
  );
  ...
  console.log(`Wrote items dataset: ${OUTPUT_PATH}`);
  console.log(`Sections: ${dataset.totalSections}`);
  console.log(`Items: ${dataset.totalItems}`);
  console.log(`Favorite Types: ${dataset.favoriteTypes.length}`);
  console.log(`Unmatched References: ${dataset.unmatchedFavoriteReferences.length}`);
}

buildItemsDataset().catch((error) => {
  console.error('Failed to build items dataset:', error);
  process.exitCode = 1;
});
